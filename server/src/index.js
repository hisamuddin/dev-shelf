...
const ok = (res, data, message = "Success", meta) => res.json({ success: true, message, data, ...(meta ? { meta } : {}), requestId: res.getHeader("X-Request-Id") });
const fail = (res, status, message, errors) => res.status(status).json({ success: false, message, ...(errors ? { errors } : {}), requestId: res.getHeader("X-Request-Id") });
...
app.get("/api/v1/resources", (req, res) => { const key = `resources:${JSON.stringify(Object.fromEntries(Object.entries(req.query).sort(([a], [b]) => a.localeCompare(b))))}`; const cached = cache.get(key); if (cached) return ok(res, cached, "Resources fetched from cache."); const data = store.publicResources(req.query); cache.set(key, data); return ok(res, data, "Resources fetched successfully."); });
app.get("/api/v1/resources/suggestions", (req, res) => ok(res, store.resourceSuggestions(req.query.q, req.query.limit), "Suggestions fetched successfully."));
app.get("/api/v1/resources/featured", (req, res) => { const key = "resources:featured"; const cached = cache.get(key); if (cached) return ok(res, cached, "Featured resources fetched from cache."); const data = store.publicResources({ featured: "true", sort: "recent", pageSize: 50 }).items; cache.set(key, data); return ok(res, data); });
app.get("/api/v1/resources/trending", (req, res) => { const key = "resources:trending"; const cached = cache.get(key); if (cached) return ok(res, cached, "Trending resources fetched from cache."); const data = store.publicResources({ sort: "trending", pageSize: 5 }).items; cache.set(key, data); return ok(res, data); });
...
app.post("/api/v1/admin/submissions/:id/:action", requireAuth, requireRole("admin"), (req, res) => { const submission = store.submissions.find((s) => s.id === req.params.id); if (!submission) return fail(res, 404, "Submission not found."); const actions = { approve: "approved", reject: "rejected", request_changes: "changes_requested" }; if (!actions[req.params.action]) return fail(res, 404, "Unsupported moderation action."); submission.status = actions[req.params.action]; submission.reviewNote = req.body.note || (submission.status === "rejected" ? "Please revise and resubmit." : "Looks good. Ready to publish."); submission.updatedAt = new Date().toISOString(); return ok(res, submission, "Moderation action recorded."); });
